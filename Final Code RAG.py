...
def _ensure_rag_index():
    """
    load a simple RAG index from the example lessons file provided by the user

      outputs stored in session_state:
      - st.session_state["rag_vectors"]: np.ndarray of embeddings with shape (N, D),
        where N = number of usable rows and D = embedding dimension.
      - st.session_state["rag_meta"]: list of dicts containing the text + attributes
        for each embedded row. This is used later to reconstruct retrieved examples.

    Returns:
      (vectors, meta) if successful, else (None, None).
    """
    df_rag = st.session_state.get("rag_df")
    if df_rag is None or df_rag.empty:
        return None, None

    # If we've already built embeddings + metadata before in this session,
    # just return the cached version to avoid re-embedding every run.
    ...
    # Auto-detect relevant column names in the RAG CSV, incase of different naming
    ...
    # Iterate through each example row in the RAG dataset, this cleans the text fields
    for _, row in df_rag.iterrows():
        ...
        # Embedding the full (topic + source + sample lesson) makes similarity search
        # and match the user's input to relevant examples
...
# --- Main Layout ---
# The first column is used for the setup and file importing + topic and source, coloumn 2 is for the output and evaluation
